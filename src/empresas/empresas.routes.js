...
import { empresaPost, empresaGet, empresaPut } from './empresas.controller.js';
...
router.get("/:order", [validarJWT, validarCampos], empresaGet);
// Funcion por si no se manda la orden
router.get("/", [validarJWT, validarCampos], empresaGet);

router.put(
  "/:id",
  [
      check("id", "Este id no es un formato válido de MongoDB").isMongoId(),
